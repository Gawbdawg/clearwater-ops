...
const { buildReceiptPdf, pdfDocToBuffer } = require('./receiptPdf');
...
Clear Water Spa Service`;
    ...
    // rental owners THEY work for and need something they can actually hand over as
    // proof — a PDF with the line items broken out, not just a summary sentence.
    let attachments;
    try {
      const pdfDoc = buildReceiptPdf({ invoice, recipient, property, paidDate, businessName: 'Clear Water Spa Service' });
      const pdfBuffer = await pdfDocToBuffer(pdfDoc);
      attachments = [{ filename: `receipt-invoice-${invoice.id}.pdf`, content: pdfBuffer, contentType: 'application/pdf' }];
    } catch (pdfErr) {
      // Never let a PDF-generation hiccup block the receipt email itself — the
      // plain-text body already has everything, the PDF is a bonus.
      console.error(`Could not build receipt PDF for invoice #${invoice.id}:`, pdfErr.message);
    }
...
module.exports = { sendPaymentReceipt, propertyReference };
