...
function buildInvoicePdf({ invoice, recipient, property, payLink, businessName, businessAddress, businessPhone, businessEmail }) {
  const doc = new PDFDocument({ margin: 54, size: 'LETTER' });

  doc.fontSize(16).font('Helvetica-Bold').text(businessName, { align: 'center' });
  // Business contact info, shown right under the name so a homeowner can reach Clear
  // Water directly from a forwarded/printed invoice. All three are optional (kept as
  // separate params rather than folded into businessName) so this still renders
  // cleanly for a caller that doesn't have them on hand.
  if (businessAddress) {
    doc.moveDown(0.15);
    doc.fontSize(9).font('Helvetica').fillColor('#555').text(businessAddress, { align: 'center' });
  }
  const contactLine = [businessPhone, businessEmail].filter(Boolean).join('   ·   ');
  if (contactLine) {
    doc.moveDown(0.05);
    doc.fontSize(9).font('Helvetica').fillColor('#555').text(contactLine, { align: 'center' });
  }
  doc.fillColor('#000');
  doc.moveDown(0.3);
