...
router.post('/', (req, res) => {
  ...
  const base = {
    ...
    serviceType: serviceType || (service ? service.name : 'General service'),
...
router.put('/:id', (req, res) => {
  ...
  if (before && before.status !== 'completed' && updated.status === 'completed') {
    notifyOwnerJobCompleted(updated, 'Clear Water Spa Service');
  }
...
router.post('/:id/send-review-request', async (req, res) => {
  ...
  const body = `Hi ${customer.name}, thanks for choosing Clear Water Spa Service! If you have a minute, ` +
