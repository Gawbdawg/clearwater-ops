...
const RIPPLE_SYSTEM = 'You are Ripple, the AI assistant built into Clear Water Spa Service\'s office dashboard, a small hot tub/spa maintenance business. Write plainly and briefly, like a sharp office manager, not a chatbot. No greetings, no "I hope this helps," no markdown formatting, no bullet points unless asked. Just the requested text.';
...
// `forOwner: true` (owner portal's "Last visit" card) writes in the warmer, second-
// person owner voice — same as generateOwnerBriefing — instead of the office-facing
// default the admin People profile uses.
async function generateVisitSummary(raw, { forOwner = false } = {}) {
  // If there's nothing here to summarize (no notes, no chemistry — e.g. a visit that
  // was marked complete without a tech filling anything in), don't hand an empty
  // object to Claude at all. Past behavior was a real bug: the model would correctly
  // notice there was nothing to summarize and say so in its own words — something
  // like "I need the actual visit notes and chemistry readings to write a summary" —
  // which then got shown to the owner verbatim as if it were their visit summary.
  // The deterministic template already has a clean, correct answer for this case
  // ("Visit completed, no notes recorded."), so just use that directly.
  const hasContent = !!(raw && (raw.notes || raw.chlorine || raw.ph || raw.alkalinity));
  if (!hasContent) return { text: templateVisitSummary(raw || {}), aiGenerated: false };
  return withFallback(
    ...
    () => callClaude(
      ...
      { maxTokens: 150, system: forOwner ? RIPPLE_OWNER_SYSTEM : RIPPLE_SYSTEM }
    )
  );
}
