...
// Auto-scheduled turnover cleanings default to Maxwell (matched by name, not a
// hardcoded id, so this keeps working if his technician record is ever recreated) —
// every iCal-derived checkout should land directly on his schedule instead of sitting
// unassigned. Falls back to null (unassigned, same as before) if no technician named
// Maxwell exists yet, so this never crashes an account that hasn't set him up.
function defaultCheckoutTechnicianId() {
  const maxwell = store.getAll('technicians').find(
    (t) => (t.name || '').trim().toLowerCase() === 'maxwell'
  );
  return maxwell ? maxwell.id : null;
}
...
function maybeCreateCheckoutAppointment(customerId, checkoutDate) {
  ...
  return store.create('appointments', {
    customerId,
    technicianId: defaultCheckoutTechnicianId(),
    date: checkoutDate,
...
// One-time (but safe-to-repeat) fix for auto-scheduled checkouts that predate the
// Maxwell default above, or otherwise ended up unassigned — reassigns any still-
// 'scheduled' auto-scheduled visit (matched by its fixed note) with no technician to
// Maxwell. Never touches a visit someone deliberately assigned to a different tech —
// only ones sitting unassigned.
function assignDefaultTechnicianToUnassigned() {
  const technicianId = defaultCheckoutTechnicianId();
  if (!technicianId) return { reassigned: 0, reason: 'No technician named Maxwell found.' };
  const appts = store.getAll('appointments').filter((a) => (
    a.status === 'scheduled'
    && a.notes === 'Auto-scheduled: guest checkout day.'
    && !a.technicianId
  ));
  appts.forEach((a) => store.update('appointments', a.id, { technicianId }));
  return { reassigned: appts.length };
}
...
module.exports = {
  ...
  assignDefaultTechnicianToUnassigned,
};
