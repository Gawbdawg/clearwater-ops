const state = { customers: [], owners: [], technicians: [], appointments: [], invoices: [] };
...
async function loadOwners() {
  ...
  `).join('') || '<tr><td colspan="7" class="empty-state">No owner accounts yet. Create one from the Customers tab when editing a property.</td></tr>';
}

function ownerForm(o = {}) {
  return `
    <label>Name<input id="f_oname" value="${o.name || ''}" /></label>
...
document.getElementById('newOwnerBtn').addEventListener('click', () => {
  openModal('New Owner', ownerForm());
    ...
    ? appts.map((a) => `
        <div class="appt-card">
          ...
          <div style="display:flex; align-items:center; gap:6px; flex-wrap:wrap;">
            <span class="badge ${a.status}">${a.status}</span>
            ${a.customerPhone ? `<button class="btn small" onclick="sendReminder(${a.id})">${a.reminderSentAt ? 'Re-send text' : 'Text reminder'}</button>` : ''}
...
function apptForm(a = {}) {
  ...
  return `
    <label>Customer
      <select id="f_customerId">${customerOptions(a.customerId)}</select>
    </label>
    <label>Technician
...
document.getElementById('syncAllBtn').addEventListener('click', async () => {
  ...
  try {
    const { results } = await api('/api/bookings/sync-all', { method: 'POST' });
    if (results.length === 0) {
      statusEl.textContent = 'No properties have an iCal link set yet.';
    } else {
      const okCount = results.filter((r) => r.ok).length;
      const failed = results.filter((r) => !r.ok);
      statusEl.textContent = `Synced ${okCount}/${results.length} propert${results.length === 1 ? 'y' : 'ies'}.` +
        (failed.length ? ` Failed: ${failed.map((f) => f.customerName).join(', ')}.` : '');
    }
    loadBookings();
  } catch (e) {
