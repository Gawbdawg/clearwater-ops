...
// appointments with serviceId: null. Left alone, that means computeBillFor can't price
// them and nothing ever gets invoiced — silently, since a tech marking a job complete
// gets no error either way. This looks at the same customer's most recent OTHER
// appointment that DOES have a service linked, on the theory that a customer's service
// rarely changes visit to visit, and backfills serviceId onto this appointment so it's
// consistently priceable (and visible/editable in the admin UI) from here on. Returns
// null (no-op) if this customer has no prior appointment with a service linked either.
function ensureServiceId(appt) {
  ...
  if (!candidates.length) return appt;
  return store.update('appointments', appt.id, { serviceId: candidates[0].serviceId }) || appt;
}
