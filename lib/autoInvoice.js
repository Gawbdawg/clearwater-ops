...
// Autopay no longer fires the moment an invoice is created — a completed job's
// invoice (or a cancellation fee, or a monthly combined bill) now always lands as a
// normal draft that sits in the Invoices tab / Money queue until the admin reviews
// it. For an owner with autopay + a card on file, the admin then charges it on
// purpose with the "Process payment" button (see routes/invoices.js's
// POST /:id/process-payment, which calls lib/autopay.js#attemptAutopay directly)
// instead of "Email invoice" — nothing here creates a charge by itself anymore.
...
// noteOverride lets callers reuse this same 24-hours-notice fee for situations other
// than an outright cancellation — e.g. an owner moving a visit to a new date within
// the window (see routes/ownerPortal.js#/appointments/:id/move) is just as disruptive
// to the day's route as a cancellation would be, so it bills the same 50% fee, just
// with wording on the invoice that matches what actually happened.
function maybeCreateCancellationFeeInvoice(rawAppt, noteOverride) {
  const appt = ensureServiceId(rawAppt);
  if (!appt || !appt.serviceId) return null;
  const service = store.getById('services', appt.serviceId);
  if (!service) return null;
  ...
  const created = store.create('invoices', {
    ...
    dueDate: '',
    status: 'draft',
    notes: noteOverride || `Cancellation fee — visit cancelled within 24 hours of its scheduled time (50% of ${service.name}).`,
  });
  return created;
}
...
// Custom pricing (home-level or owner-level) can change after some of a customer's
// jobs are already billed but still sitting as a draft invoice — those drafts were
// priced under the OLD rate and would otherwise stay stuck showing a number the admin
// just changed. resolvePrice reads the customer/owner's current customPricing fresh
// every time, so simply re-running syncInvoiceForCompletedAppointment against every
// completed job for this customer picks the new price back up — same mechanism as the
// existing "addon added after completion" resync above, just triggered by a price
// change instead. Already-sent or paid invoices are left alone, exactly like that
// existing resync — once an owner's seen a number, changing it silently would be
// confusing.
function resyncDraftInvoicesForCustomer(customerId) {
  const appts = store.getAll('appointments').filter(
    (a) => a.customerId === customerId && a.status === 'completed'
  );
  let updatedCount = 0;
  appts.forEach((appt) => {
    if (syncInvoiceForCompletedAppointment(appt)) updatedCount += 1;
  });
  return updatedCount;
}

// Same, but for an owner-level custom-price change — sweeps every property that owner
// has, since an owner-level rate applies across all of them (see resolvePrice above).
function resyncDraftInvoicesForOwner(ownerId) {
  const propertyIds = store.getAll('customers')
    .filter((c) => c.ownerId === ownerId)
    .map((c) => c.id);
  return propertyIds.reduce((sum, id) => sum + resyncDraftInvoicesForCustomer(id), 0);
}

// A one-time, site-wide version of the two functions above — sweeps every customer
// so any draft invoice picks up whatever price currently resolves for it (home
// custom price, then owner custom price, then frequency-tier rate, then catalog
// default; see resolvePrice). Exists for a manual "refresh" admin action (see the
// Invoices tab's "Refresh prices to current defaults" button) to catch drafts left
// stale by a price change that predates the automatic per-save resync above, or a
// catalog/frequency-tier price change (which has no single customer/owner save to
// hang a resync off of).
function resyncAllDraftInvoices() {
  const customers = store.getAll('customers');
  return customers.reduce((sum, c) => sum + resyncDraftInvoicesForCustomer(c.id), 0);
}
...
module.exports = {
  ...
  addonsTotal,
  addonsNote,
  resyncDraftInvoicesForCustomer,
  resyncDraftInvoicesForOwner,
  resyncAllDraftInvoices,
};
