...
const COLLECTIONS = ['customers', 'owners', 'technicians', 'appointments', 'invoices', 'bookings', 'serviceRequests', 'admins', 'services', 'addons', 'techTimeOff', 'timeEntries', 'payouts'];
...
function buildDefaultData() {
  const data = {
    ...
    timeEntries: [],
    // One row per payment actually made to a technician outside this app (cash,
    // Venmo, a paper check, whatever) — e.g. { id, technicianId, amount, date,
    // note }. This is deliberately separate from timeEntries' computed "Pay"
    // figure (hours × rate): that's what's OWED for a given day; a payout is a
    // record that money actually changed hands, which the office needs to track
    // by hand since payment doesn't happen inside this app. See routes/payouts.js.
    payouts: [],
    settings: {
    ...
    nextId: { customers: 1, owners: 1, technicians: 1, appointments: 1, invoices: 1, bookings: 1, serviceRequests: 1, admins: 1, services: 1, addons: 1, techTimeOff: 1, timeEntries: 1, payouts: 1 },
  };
...
function migrate(data) {
  ...
  if (!data.settings || typeof data.settings !== 'object') {
    data.settings = { depotAddress: '1027 SW 62nd Street, Lincoln City, OR', depotLat: null, depotLng: null, notificationEmail: '' };
  }
  return data;
}
...
// In-memory cache of the parsed data.json, so getAll/getById (called constantly —
// e.g. once per line item every time an appointments/invoices/etc. list is enriched
// with related records) don't do a synchronous full-file read + JSON.parse from disk
// on every single call. This was the actual cause of "the schedule loads slowly":
// loading the Calendar view fetches every appointment ever created (no date filter),
// and each one was separately triggering two more full data.json reads from disk
// (once for its customer, once for its technician) just to fill in names/addresses —
// so the response time scaled with both the appointment count AND the file's total
// size, and kept getting worse as more history piled up.
// Safe to cache for the life of the process because this is a single Node process
// with no clustering, and store.js is the only code anywhere that ever touches
// data.json — every write goes through writeData() below, which updates this same
// cache, so nothing can silently drift out of sync with what's on disk.
let cache = null;
