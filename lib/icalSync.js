...
const { maybeCreateCheckoutAppointment, cancelConflictingCheckoutAppointments } = require('./turnoverSchedule');
...
async function syncCustomerCalendar(customerId) {
  ...
  // isn't a double-booking risk worth flagging. Skipped entirely for a property
  // marked icalMutualBlocking: true (multiple calendars there are separate rentable
  // units sharing one amenity we service — e.g. a main house + carriage house on one
  // hot tub — so overlapping dates across sources are the expected normal case, not
  // a double-booking risk. guestPresentOn/maybeCreateCheckoutAppointment already
  // union bookings across sources correctly for scheduling regardless of this flag;
  // this only controls the owner-facing "possible double-booking" warning.
  let conflictCount = 0;
  if (!customer.icalMutualBlocking) {
