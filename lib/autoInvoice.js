...
//
// Still checks the owner's custom price FIRST, same priority order as resolvePrice() —
// this used to skip straight to the tier rate, which meant bulk-fixing a backlog of
// unbilled jobs would silently override a specific owner's negotiated rate with the
// catalog's tier price picked for the batch. An owner with a custom rate set always
// bills at that rate regardless of which tier the admin picked for everyone else in the
// batch; the tier is only used as a fallback for customers with no custom price.
function computeBillForWithTier(appt, service, tier) {
  if (!appt || !service) return null;
  const { price: resolvedPrice, isCustom } = resolvePrice(service, appt.customerId);
  let price;
  let priceNote;
  if (isCustom) {
    price = resolvedPrice;
    priceNote = ', owner price';
  } else {
    const rates = service.frequencyPrices || {};
    price = rates[tier];
